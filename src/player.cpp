...
#include <godot_cpp/classes/input_event_key.hpp> // <-- Adicionado para capturar o teclado
...
void Player::_unhandled_input(const Ref<InputEvent> &event) {
    ...
    if (motion.is_valid()) {
       ...
       constexpr float HALF_PI = static_cast<float>(Math_PI) * 0.5f;

       _pitch = Math::clamp(_pitch, -HALF_PI, HALF_PI);

       set_rotation(Vector3(0, _yaw, 0));
       _camera->set_rotation(Vector3(_pitch, 0, 0));
    }
    ...
    if (key_event.is_valid() && key_event->is_pressed() && !key_event->is_echo()) {
        ...
        if (keycode >= Key::KEY_0 && keycode <= Key::KEY_9) {
            _selected_block_id = keycode - Key::KEY_0;
        }
    }
    // -------------------------------------------------------
    ...
    if (mouse_button.is_valid() && mouse_button->is_pressed()) {
       ...
       if (mouse_button->get_button_index() == MouseButton::MOUSE_BUTTON_RIGHT) {
          ...
          if (!hit.is_empty()) {
             ...
             if (auto *world = get_node<World>("../World")) {
             	BlockType selected_type = static_cast<BlockType>(_selected_block_id);

             	block::Block block_to_place = block::make_block(selected_type);
                world->set_block(position, block_to_place);
             }
          }
       }
    }
