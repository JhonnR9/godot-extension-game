...
int ChunkMeshBuilder::_get_tex_layer(const CubeFace& face, const BlockType& type) {
...
void ChunkMeshBuilder::_initialize_texture_map() {
	...
	for (int i = 0; i < keys.size(); i++) {
		...
		BlockType type = map_string_to_type(base_name);
...
godot::BlockType godot::ChunkMeshBuilder::map_string_to_type(const godot::String &name) {
